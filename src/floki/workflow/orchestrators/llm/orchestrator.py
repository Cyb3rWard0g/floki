from floki.workflow.orchestrators.llm.schemas import TriggerAction, NextStep, AgentTaskResponse, ProgressCheckOutput, PLAN_SCHEMA, NEXT_STEP_SCHEMA, PROGRESS_CHECK_SCHEMA
from floki.workflow.orchestrators.llm.prompts import TASK_INITIAL_PROMPT, TASK_PLANNING_PROMPT, NEXT_STEP_PROMPT, PROGRESS_CHECK_PROMPT, SUMMARY_GENERATION_PROMPT
from floki.workflow.orchestrators.llm.state import LLMWorkflowState, LLMWorkflowEntry, LLMWorkflowMessage, PlanStep, TaskResult
from floki.workflow.orchestrators.llm.utils import update_step_statuses, restructure_plan, find_step_in_plan
from floki.types import DaprWorkflowContext, BaseMessage, EventMessageMetadata
...
class LLMOrchestrator(OrchestratorServiceBase):
    ...
    def llm_workflow(self, ctx: DaprWorkflowContext, input: TriggerAction):
        ...
        self.state.setdefault("instances", {}).setdefault(instance_id, LLMWorkflowEntry(input=task).model_dump(mode="json"))
        ...
        valid_step = yield ctx.call_activity(self.validate_next_step, input={"instance_id": instance_id, "plan": plan, "step": step_id, "substep": substep_id})

        if valid_step:
            # Step 6: Broadcast Task to all Agents
            yield ctx.call_activity(self.broadcast_message_to_agents, input={"instance_id": instance_id, "task": instruction})
            ...
            logger.warning(f"Step {step_id}, Substep {substep_id} not found in plan for instance {instance_id}. Recovering...")
        ...
        if verdict != "continue" or next_iteration_count > self.max_iterations:
            ...
            summary = yield ctx.call_activity(self.generate_summary, input={"task": task, "verdict": verdict, "plan": plan, "step": step_id, "substep": substep_id, "agent": next_agent, "result": task_results["content"]})
    ...
    @task
    async def validate_next_step(self, instance_id: str, plan: List[Dict[str, Any]], step: int, substep: Optional[float]) -> bool:
        """
        ...
        if not step_entry:
            logger.error(f"Step {step}, Substep {substep} not found in plan for instance {instance_id}.")
            return False
        return True
    ...
    async def trigger_agent(self, instance_id: str, name: str, step: int, substep: Optional[float]) -> List[dict[str, Any]]:
        ...
        """
        logger.info(f"Triggering agent {name} for step {step}, substep {substep} (Instance ID: {instance_id})")
        ...
        if not step_entry:
            if substep is not None:
                raise ValueError(f"Substep {substep} in Step {step} not found in the current plan.")
    ...
    async def update_task_history(self, instance_id: str, agent: str, step: int, substep: Optional[float], results: Dict[str, Any]):
        ...
        logger.info(f"Updating task history for {agent} at step {step}, substep {substep} (Instance ID: {instance_id})")
        ...
        task_result = TaskResult(agent=agent, step=step, substep=substep, result=results["content"])

        # Append the result to task history
        workflow_entry["task_history"].append(task_result.model_dump(mode="json"))

        # Persist state
        await self.update_workflow_state(instance_id=instance_id, plan=workflow_entry["plan"])
    ...
    async def update_plan(self, instance_id: str, plan: List[Dict[str, Any]], status_updates: Optional[List[Dict[str, Any]]] = None, plan_updates: Optional[List[Dict[str, Any]]] = None):
        ...
        if status_updates:
            for update in status_updates:
                ...
                logger.info(f"Updated status of step {step_id}, substep {substep_id} to '{new_status}'")
    ...
    async def finish_workflow(self, instance_id: str, plan: List[Dict[str, Any]], step: int, substep: Optional[float], verdict: str, summary: str):
        ...
        if verdict == "completed":
            ...
            step_entry = find_step_in_plan(plan, step, substep)
            if not step_entry:
                raise ValueError(f"Step {step}, Substep {substep} not found in the current plan. Cannot mark as completed.")
